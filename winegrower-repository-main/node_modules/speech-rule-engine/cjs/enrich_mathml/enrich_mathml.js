...
exports.createsCycle = createsCycle;
...
function walkTree(semantic) {
    ...
    if (newNode === null) {
        ...
        if (attached) {
            ...
            if (attachedParent && semantic.parent && attached !== newNode && SemanticUtil.isDescendant(newNode, attachedParent)) {
                debugger_js_1.Debugger.getInstance().output('Walktree Case 2.1.fallback');
                ...
                if (innerNode !== newNode &&
                    ...
                    debugger_js_1.Debugger.getInstance().generate(() => ['WALKING END (4): ', semantic.toString()]);
                    return ascendNewNode(newNode, semantic);
                }
                else {
                    newNode = innerNode;
    ...
    debugger_js_1.Debugger.getInstance().generate(() => ['WALKING END (5): ', semantic.toString()]);
    return ascendNewNode(newNode, semantic);
}
function introduceNewLayer(children, semantic) {
    ...
    if (info !== lcaType.VALID ||
        ...
        if (info === lcaType.PRUNED) {
            ...
            if (node) {
                debugger_js_1.Debugger.getInstance().output('Walktree Case 1.1.1.0');
                ...
                debugger_js_1.Debugger.getInstance().output('Walktree Case 1.1.1.1');
...
function specialPostfixCase(semantic, children) {
    if (semantic.type !== 'postfixop')
        return;
    const last = children[children.length - 1];
    const sibling = last.nextSibling;
...
function mergeChildren(node, newChildren, semantic) {
    ...
    let oldCounter = 0;
    const guard = (0, semantic_util_js_1.loopGuard)(semantic_util_js_1.LOOP_LIMIT, 'mergeChildren infinite loop');
    while (newChildren.length) {
        guard();
...
function insertNewChild(node, oldChild, newChild) {
    ...
    const guard = (0, semantic_util_js_1.loopGuard)(semantic_util_js_1.LOOP_LIMIT, () => `insertNewChild loop: next=${next === null || next === void 0 ? void 0 : next.tagName} parent=${parent === null || parent === void 0 ? void 0 : parent.tagName}`);
    while (next &&
...
function createsCycle(node, newChild) {
...
function pathToRoot(node, opt_test) {
    ...
    const path = [node];
    const guard = (0, semantic_util_js_1.loopGuard)(semantic_util_js_1.LOOP_LIMIT, () => { var _a; return `pathToRoot infinite loop at ${node.tagName} parent=${(_a = node.parentNode) === null || _a === void 0 ? void 0 : _a.tagName}`; });
...
function hasSiblingInDirection(node, direction) {
    let sib = node[direction];
    while (sib) {
        if (!isIgnorable(sib)) {
            return true;
        }
...
function ascendNewNode(newNode, semantic) {
    ...
    const ceiling = (_a = semantic === null || semantic === void 0 ? void 0 : semantic.parent) === null || _a === void 0 ? void 0 : _a.mathmlTree;
    const guard = (0, semantic_util_js_1.loopGuard)(semantic_util_js_1.LOOP_LIMIT, () => { var _a; return `ascendNewNode infinite loop at ${newNode.tagName} parent=${(_a = newNode.parentNode) === null || _a === void 0 ? void 0 : _a.tagName}`; });
    ...
    while (newNode !== ceiling && !SemanticUtil.hasMathTag(newNode) &&
        ...
        (unitChild(newNode) ||
            (empty && newNode.parentNode &&
                ...
                empty.includes((_b = parentNode(newNode).tagName) === null || _b === void 0 ? void 0 : _b.toUpperCase()) &&
                onlyChild(newNode)))) {
        guard();
