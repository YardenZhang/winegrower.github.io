...
export function isStructuralParent(node) {
    return STRUCTURAL.includes(DomUtil.tagName(node));
}
export const LOOP_LIMIT = 10000;
export function loopGuard(limit, message) {
    let count = 0;
    return () => {
        if (++count > limit) {
...
export function isDescendant(descendant, node) {
    if (!descendant) {
        return false;
    }
...
export function meaningfulSpace(node) {
